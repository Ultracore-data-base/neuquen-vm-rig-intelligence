...
<div class="uc-top">
  ...
  <div>
    <div class="uc-brand-main">ULTRACORE</div>
    <div class="uc-brand-sub">ENERGY INTELLIGENCE</div>
  </div>
</div>
""", unsafe_allow_html=True)
    ...
    with map_col:
        ...
        if folium is None or st_folium is None:
            ...
            if not gis_layers.empty and {"url", "layer_id", "layer_name"}.issubset(gis_layers.columns):
                for _, layer_row in gis_layers.iterrows():
                    try:
                        folium.raster_layers.WmsTileLayer(
                            url=str(layer_row["url"]),
                            layers=str(layer_row["layer_id"]),
                            name=f"{layer_row.get('province','')} - {layer_row.get('layer_name', layer_row['layer_id'])}",
                            fmt="image/png",
                            transparent=True,
                            version=str(layer_row.get("version", "1.1.1")),
                            overlay=True,
                            control=True,
                            show=False,
                        ).add_to(m)
